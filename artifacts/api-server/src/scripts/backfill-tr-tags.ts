/**
 * One-shot backfill: re-fetch Radio-Browser tags for Turkish stations whose
 * `tags` field is missing or empty (TR audit P2).
 *
 * Re-uses `SyncService.hydrateMissingTagsInBackground({ countryCode: 'TR' })`
 * so the script and the in-process background job follow exactly the same
