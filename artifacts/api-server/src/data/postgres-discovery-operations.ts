...
export async function pgCityCounts(
  ...
  const rows = (
    ...
    SELECT name,tags_raw FROM stations WHERE (is_list_visible IS TRUE OR COALESCE(visibility_expires_at<=now(),false)) AND lower(country)=ANY($1::text[])
...
export async function pgGlobalCityCounts(
  ...
  return (
    ...
    AND (s.is_list_visible IS TRUE OR COALESCE(s.visibility_expires_at<=now(),false)) AND strpos(lower(s.state),lower(c.name))>0 GROUP BY c.name,c.country ORDER BY count(s.id) DESC,c.name LIMIT 20`,
...
export async function pgDiverseStations(
  ...
  const rows = (
    ...
    SELECT s.id FROM stations s WHERE (s.is_list_visible IS TRUE OR COALESCE(s.visibility_expires_at<=now(),false)) AND ($1::text IS NULL OR lower(s.country)=lower($1))
  ...
  const fields = [
    ...
    "bitrate",
    "lastCheckOk",
    "isListVisible", "availabilityStatus",
  ];
