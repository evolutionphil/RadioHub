...
export function registerWebPushRoutes(
  app: Express,
  deps: { requireAuth: (req: Request, res: Response, next: () => void) => void },
): void {
  ...
  app.post('/api/user/push-subscription', requireAuth, async (req: any, res) => {
    try {
      ...
      await User.updateOne(
        { _id: req.user._id },
        { $set: { pushSubscription: { endpoint: sub.endpoint, keys: { p256dh: sub.keys.p256dh, auth: sub.keys.auth } } } },
      );
  ...
  app.delete('/api/user/push-subscription', requireAuth, async (req: any, res) => {
    try {
      await User.updateOne({ _id: req.user._id }, { $unset: { pushSubscription: 1 } });
      res.json({ success: true });
    } catch (error: any) {
