...
before(async () => {
  ...
  fixture = await createNativePostgresFixture("coverage-drop-acknowledge");
...
beforeEach(async () => {
  ...
  await fixture.clear("admin_settings", "user_notifications", "users");
});
...
async function insertAlert(snapshotDate: string, opts?: { createdAt?: Date }) {
  ...
  return fixture.insert("user_notifications", {
    userId,
    type: "system",
    title: "Coverage drop",
...
test("POST /acknowledge returns 400 when snapshotDate is missing", async () => {
  ...
  const stored = await pgAdminSettings().get(ACK_KEY);
