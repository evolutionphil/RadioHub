 ...
 * registered against an isolated PostgreSQL schema so we exercise the actual
 * native SQL stores + handler logic end-to-end.
 */
...
import { pgAdminSettings } from "../src/data/postgres-admin-settings-store";
...
before(async () => {
  ...
  fixture = await createNativePostgresFixture("coverage-drop-settings-history");
...
beforeEach(async () => {
  ...
  invalidateCoverageDropSettingsCache();
  await fixture.clear("admin_settings", "admin_setting_history");
});
...
test("PUT writes an AdminSettingHistory update row with previousValue=null on first save", async () => {
  ...
  const rows = await readHistory(-1);
...
test("DELETE writes an AdminSettingHistory clear row with the prior value preserved", async () => {
  ...
  const remaining = await pgAdminSettings().get(COVERAGE_DROP_SETTINGS_KEY);
...
test("Reverting an entry whose previousValue is null clears the override", async () => {
  ...
  const stored = await pgAdminSettings().get(COVERAGE_DROP_SETTINGS_KEY);
  ...
  const body = (await (await fetch(`${baseUrl}${SETTINGS_URL}`)).json()) as {
    ...
    effective: {
      source: { thresholdPp: string; minStations: string; webhookUrl: string };
    };
  };
...
async function readHistory(order: number): Promise<any[]> {
  return (
    await fixture.pool.query(
      'SELECT action,previous_value AS "previousValue",new_value AS "newValue",changed_by AS "changedBy",changed_at AS "changedAt" FROM admin_setting_history WHERE key=$1 ORDER BY changed_at ' +
        (order === 1 ? "ASC" : "DESC") +
        ",id " +
        (order === 1 ? "ASC" : "DESC"),
      [COVERAGE_DROP_SETTINGS_KEY],
...
async function insertHistoryFixtures(rows: any[]) {
  for (const row of rows) await fixture.insert("admin_setting_history", row);
}
