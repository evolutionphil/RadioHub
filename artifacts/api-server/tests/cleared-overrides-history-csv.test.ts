...
// ---------------------------------------------------------------------------
// Source-shaped scenarios persisted into an isolated PostgreSQL schema.
// ---------------------------------------------------------------------------
...
// Persist the same audit scenarios into PostgreSQL before invoking the real
// route. SQL filters and JSONB expansion are not emulated by test-side queries.
async function persistAuditFixtures(): Promise<void> {
  for (const audit of audits) {
    if (persistedIds.has(audit._id)) continue;
    await fixture.insert("country_language_mapping_audit", audit);
    persistedIds.add(audit._id);
  }
}
...
before(async () => {
  ...
  fixture = await createNativePostgresFixture("cleared-overrides-csv");
...
const CSV_PATH =
  "/api/admin/country-language-mappings/cleared-overrides-log/all/csv";
...
test("CSV unfiltered: header + one row per snapshot entry, plus one summary row per empty-snapshot entry", async () => {
  ...
  const editPlaceholder = rows.find((r) => r[0] === "2026-03-01T09:00:00.000Z");
  ...
  const bulkPlaceholder = rows.find((r) => r[0] === "2026-03-20T18:45:00.000Z");
...
test("CSV ?action=clear-overrides narrows to that audit type only and matches the list endpoint count", async () => {
  ...
  assert.equal(
    distinctEntries,
    await fetchListTotal("?action=clear-overrides"),
  );
});
...
test("CSV ?country= matches snapshot countryCode AND inline change.countryCode", async () => {
  ...
  assert.equal(
    listTotal,
    2,
    "list endpoint should match 2 audit entries for DE",
  );
...
test("CSV with only empty-snapshot entries still emits one summary row each", async () => {
  ...
  assert.deepEqual(noop, ["2026-04-02T00:00:00.000Z", "", "0", "", "", "", ""]);
});
