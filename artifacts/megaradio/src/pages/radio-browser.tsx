...
export default function RadioBrowser() {
  ...
  const [submittedSearch, setSubmittedSearch] = useState<Record<string, string | boolean> | null>(null);
  ...
  const { data: trendingStations, isLoading: loadingTrending, isError: trendingError } = useQuery<{stations: any[]}>({
    queryKey: ['/api/admin/radio-browser/stations', { kind: 'trending', limit: 50 }],
    enabled: activeTab === 'trending'
  });

  const { data: recentStations, isLoading: loadingRecent, isError: recentError } = useQuery<{stations: any[]}>({
    queryKey: ['/api/admin/radio-browser/stations', { kind: 'recent', limit: 50 }],
    enabled: activeTab === 'recent'
  });

  const { data: qualityStations, isLoading: loadingQuality, isError: qualityError } = useQuery<{stations: any[]}>({
    queryKey: ['/api/admin/radio-browser/stations', { kind: 'quality', limit: 50 }],
    enabled: activeTab === 'quality'
  });

  const { data: topClickedApi, isLoading: loadingTopClicked, isError: clickedError } = useQuery<{stations: RadioBrowserStation[]}>({
    queryKey: ['/api/radio-browser/top-clicked', { limit: 100 }],
    enabled: activeTab === 'api-top-clicked'
  });

  const { data: topVotedApi, isLoading: loadingTopVoted, isError: votedError } = useQuery<{stations: RadioBrowserStation[]}>({
    queryKey: ['/api/radio-browser/top-voted', { limit: 100 }],
    enabled: activeTab === 'api-top-voted'
  });

  const { data: recentApi, isLoading: loadingRecentApi, isError: recentApiError } = useQuery<{stations: RadioBrowserStation[]}>({
    queryKey: ['/api/radio-browser/recent', { limit: 100 }],
    enabled: activeTab === 'api-recent'
  });

  const { data: brokenApi, isLoading: loadingBroken, isError: brokenError } = useQuery<{stations: RadioBrowserStation[]}>({
  ...
  const { data: searchResults, refetch: searchStations, isFetching: loadingSearch, isError: searchError } = useQuery<{stations: RadioBrowserStation[], total: number}>({
    queryKey: ['/api/admin/radio-browser/stations', submittedSearch],
    enabled: activeTab === 'search' && submittedSearch !== null
  });

  const handleSearch = async () => {
    const cleanParams = Object.fromEntries(Object.entries(searchParams).filter(([, value]) => value !== ''));
    if (JSON.stringify(cleanParams) === JSON.stringify(submittedSearch)) await searchStations();
    else setSubmittedSearch(cleanParams);
  };

  const handleStationClick = async (station: any) => {
    try {
      await playStation({ ...station, _id: station._id || station.stationuuid,
        urlResolved: station.urlResolved || station.url_resolved || station.url });
    } catch {
      toast({ title: 'Playback failed', description: 'Unable to play this stream.', variant: 'destructive' });
    }
  };

  const handleStationVote = async (stationId: string, stationName: string) => {
    try {
      const response = await apiRequest('POST', `/api/stations/${encodeURIComponent(stationId)}/vote`);
    ...
    <Card className="hover:shadow-md transition-shadow">
      ...
      <CardContent className="pt-0">
        <div className="flex flex-wrap gap-1 sm:gap-2 mb-2 sm:mb-3">
          ...
          {(station.availabilityStatus === 'working') && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
            <span className="flex items-center gap-1">
              ...
              <span className="hidden sm:inline">{station.clickCount ?? station.clickcount ?? 0} clicks</span>
              <span className="sm:hidden">{station.clickCount ?? station.clickcount ?? 0}</span>
            </span>
            <span className="flex items-center gap-1">
  ...
  const renderStationGrid = (stations: any, loading: boolean, isApiStation = false) => {
    ...
    if (!stationList.length) return <p className="text-muted-foreground py-6">No stations match these filters.</p>;
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] px-4 py-5 sm:px-6 sm:py-6 lg:px-8 space-y-5 sm:space-y-6">
      ...
      {({trending: trendingError, recent: recentError, quality: qualityError, 'api-top-clicked': clickedError, 'api-top-voted': votedError, 'api-recent': recentApiError, 'api-broken': brokenError, search: searchError} as Record<string, boolean>)[activeTab] && <p role="alert" className="text-destructive">Station data could not be loaded. Select the tab again or retry the search.</p>}
      {/* API Statistics Card */}
      {apiStats && (
        <Card className="mb-6">
          <CardHeader>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-1">
          ...
          <TabsTrigger value="api-recent" className="text-xs sm:text-sm hidden lg:block">Synced Recent</TabsTrigger>
        ...
        <TabsContent value="api-top-clicked" className="mt-6">
          <div className="mb-4">
            <h2 className="text-xl font-semibold mb-2">Top Clicked (Synced Catalogue)</h2>
            <p className="text-muted-foreground">Most clicked stations from the synchronized PostgreSQL catalogue</p>
          </div>
        ...
        <TabsContent value="api-top-voted" className="mt-6">
          <div className="mb-4">
            <h2 className="text-xl font-semibold mb-2">Top Voted (Synced Catalogue)</h2>
            <p className="text-muted-foreground">Highest voted stations from the synchronized PostgreSQL catalogue</p>
          </div>
        ...
        <TabsContent value="api-recent" className="mt-6">
          <div className="mb-4">
            <h2 className="text-xl font-semibold mb-2">Recently Changed (Synced Catalogue)</h2>
            <p className="text-muted-foreground">Latest station changes from the synchronized PostgreSQL catalogue</p>
          </div>
        ...
        <TabsContent value="search" className="mt-6">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">Search the synchronized catalogue with comprehensive filters</p>
          </div>
