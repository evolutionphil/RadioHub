...
- **Memory Management**: Multi-layer OOM prevention including heap monitoring and cache clearing with rate-limiting and intelligent key eviction. Heap limit: 4096MB (32GB server). Thresholds: 3000MB=proactive clearSeoAndQuickCaches; 3500MB=critical clearAllForMemoryRelief. Semi-space: 64MB for reduced minor GC pauses. Operation tracker (`server/utils/operation-tracker.ts`) monitors active operations and GC pauses via PerformanceObserver — diagnostics visible at `/health` or `/api/health` (not `/healthz` which returns plain `ok` for Railway speed). Sharp memory capped: `cache({memory:50,files:20,items:100})`, `concurrency(1)`. Global JSON body limit: 2MB (50MB only on `/api/admin/bulk-import-stations`). Compression level: 1 (was 6, blocked event loop). Node flags applied via `start.sh` (railway.toml `startCommand`).\n\nIMPORTANT: Railway uses Dockerfile for builds. Node flags MUST be in BOTH Dockerfile CMD AND railway.toml startCommand. start.sh doesn't work because Dockerfile multi-stage build doesn't copy it to production stage.
- **SEO Render Protection**: Max 3 concurrent SSR renders, 8s timeout per render, 30s HTTP request timeout. Prevents Googlebot crawl storms from cascading into server-wide event loop blocks.
